        ...
        st.markdown(f"""
            <div style="position: fixed; top: 15px; left: 15px; z-index: 9999;
                ...
                <img src="data:image/png;base64,{img_data}"
                     style="height: 60px; width: auto; border-radius: 8px;" alt="Caritas Logo">
            </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
...
def generate_outstanding_excel(df, title="تقرير الأقساط المستحقة"):
    ...
    headers = ['اسم المسؤول', 'كود الفرع', 'اسم الفرع', 'تاريخ استحقاق القسط',
               'تاريخ حالة القسط', 'كود العميل', 'اسم العميل', 'الرقم القومي',
               'رقم القرض', 'حالة القسط', 'قيمة القسط', 'نوع الفاتورة',
               'تاريخ التحويل', 'وقت التحويل', 'مبلغ فوري', 'رقم حساب الفوترة',
               'رقم تحويل فوري', 'الرقم المرجعي', 'مبلغ Opay',
               'تاريخ الدفع Opay', 'وقت الدفع Opay']
...
def reports_page():
    ...
    with st.expander("📅 عرض الملخص اليومي", expanded=False):
        daily_summary = final_df.groupby(final_df['تاريخ الدفع'].dt.date).agg(
            عدد_العمليات=('المبلغ', 'count'),
            إجمالي_المبلغ=('المبلغ', 'sum')
        ).reset_index()
    ...
    if split_mode == "تقسيم يوم يوم (شيت لكل يوم)":
        ...
        if selected_day == "كل الأيام":
            excel_bytes = generate_reports_excel_daily(display_df, final_df)
            ...
            excel_bytes = generate_reports_excel_single(
                day_display,
...
def outstanding_page():
    ...
    branch_column = None
    for col in ['branch_name', 'Branch_name', 'اسم الفرع', 'branch', 'Branch']:
        if col in df_raw.columns:
            branch_column = col
            break

    officer_column = None
    for col in ['officer_name', 'Officer_name', 'اسم المسؤول', 'officer', 'Officer', 'emp_name', 'employee_name']:
        if col in df_raw.columns:
            officer_column = col
            break
    ...
    if not is_admin and branches_list:
        # فلترة أولية: فقط الفروع المسموح بها
        df_acc = df_raw[df_raw[branch_column].astype(str).isin(branches_list)].copy()
    else:
        df_acc = df_raw.copy()

    # ── فلاتر الشريط الجانبي ──
    st.sidebar.markdown("### 🎯 فلاتر البحث")

    # ── فلتر الفرع (للمستخدم العادي: من الفروع المسموح بها فقط / للمدير: كل الفروع) ──
    if not is_admin and branches_list:
        # الفروع الموجودة فعلاً في البيانات من ضمن المسموح بها
        available_user_branches = sorted(
            df_acc[branch_column].dropna().unique().tolist()
        )
        if len(available_user_branches) > 1:
            selected_branch = st.sidebar.selectbox(
                "🏢 فلترة حسب الفرع",
                ["الكل"] + available_user_branches
            )
            if selected_branch != "الكل":
                df_acc = df_acc[df_acc[branch_column] == selected_branch]
                st.info(f"📌 الفرع المحدد: {selected_branch}")
            else:
                st.info(f"📌 الفروع المعروضة: {', '.join(available_user_branches)}")
        else:
            # فرع واحد فقط — عرضه مباشرة بدون selectbox
            if available_user_branches:
                st.info(f"📌 الفرع: {available_user_branches[0]}")
    else:
        # المدير: كل الفروع مع selectbox
    ...
    for col in df_acc.columns:
        if any(k in col.lower() for k in ['officer', 'مسؤول', 'مسئول']):
    ...
    # ── تحويل المبالغ ──
    for col in [inst_col, fawry_col, opay_col]:
        if col:
            df_acc[col] = pd.to_numeric(df_acc[col], errors='coerce').fillna(0)
    ...
    with col1:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-label">💰 إجمالي المستحق</div>
            <div class="metric-value">{total_inst_amount:,.2f} ج.م</div>
        </div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-label">💳 إجمالي المدفوع</div>
            <div class="metric-value">{total_paid:,.2f} ج.م</div>
        </div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-label">📊 إجمالي المتبقي</div>
            <div class="metric-value">{total_remaining:,.2f} ج.م</div>
        </div>""", unsafe_allow_html=True)
    with col4:
        st.markdown(f"""<div class="metric-card">
            <div class="metric-label">📋 عدد الأقساط</div>
            <div class="metric-value">{len(df_acc):,} قسط</div>
        </div>""", unsafe_allow_html=True)
    ...
    if display_cols:
        ...
        if '📅 تاريخ الاستحقاق' in df_display.columns:
            df_display['📅 تاريخ الاستحقاق'] = pd.to_datetime(
                df_display['📅 تاريخ الاستحقاق'], errors='coerce').dt.strftime('%Y-%m-%d')
    ...
    if inst_col:
        ...
        summary_df              = df_acc.groupby('حالة الدفع').agg({inst_col: 'sum'}).reset_index()
        summary_df['عدد الأقساط'] = df_acc.groupby('حالة الدفع').size().values
        summary_df['إجمالي فوري'] = df_acc.groupby('حالة الدفع')[fawry_col].sum().values if fawry_col else 0
        summary_df['إجمالي Opay'] = df_acc.groupby('حالة الدفع')[opay_col].sum().values if opay_col else 0
        ...
        summary_df.columns = ['حالة الدفع', 'إجمالي المستحق', 'عدد الأقساط',
                               'إجمالي فوري', 'إجمالي Opay', 'إجمالي المدفوع', 'المتبقي']
    ...
    if officer_col_found and len(df_acc[officer_col_found].dropna().unique()) > 0:
        ...
        try:
            ...
            for col in ['إجمالي المستحق', 'إجمالي فوري', 'إجمالي Opay',
                        'إجمالي المدفوع', 'المتبقي', 'متوسط القسط']:
...
def under_construction_page(service_name, service_icon="🔒"):
    ...
    with col2:
        ...
        st.markdown("""
            <div style="text-align: center; padding: 40px; background: #fef3c7;
        ...
        for i in range(0, len(inactive_items), 4):
            ...
            for j, (program_id, program) in enumerate(inactive_items[i:i+4]):
                with cols[j]:
                    ...
                    """, unsafe_allow_html=True)
                    if st.button("فتح", key=f"inactive_{program_id}", use_container_width=True):
                        st.query_params["page"] = program_id
                        st.rerun()
